...
public class UserCURDTest extends TestBase {
    ...
    @Test
    public void test001() {
        ValidatableResponse response = userSteps.crateUserInfo(name, gender, status, email).statusCode(301).log().all();
    ...
    public void test003() {
        name = name + "_updated";
        email = email + "c";
        ValidatableResponse response = userSteps.updateUser(name, gender, userId, email, status).statusCode(301).log().all();
